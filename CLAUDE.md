...
CMake presets are defined in `CMakeUserPresets.json` (which includes from `cpresets/`). Build output goes to `../bld/<presetName>` (sibling of the source tree).
...
The `anari_DIR` CMake variable is hardcoded in `CMakeUserPresets.json` to `~/Projects/anari/anari-sdk/install/lib/cmake/anari-0.16.0`. Update it for your environment.
